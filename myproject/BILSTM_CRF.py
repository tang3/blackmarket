...
class BILSTM_CRF(object):
    ...
    def __init__(self, num_chars, num_classes, num_steps=100, num_epochs=10, hidden_dim=200,num_layers=1,embedding_matrix=None, is_training=True, is_crf=True, weight=False):
        # Parameter
        self.max_f1 = 0
        ...
        lstm_cells_fw =tf.nn.rnn_cell.MultiRNNCell([lstm_cell_fw] * self.num_layers)#[lstm_cell_fw for i in range(self.num_layers)]#tf.nn.rnn_cell.MultiRNNCell([lstm_cell_fw] * self.num_layers)
        lstm_cells_bw =tf.nn.rnn_cell.MultiRNNCell([lstm_cell_bw] * self.num_layers)#[lstm_cell_bw for i in range(self.num_layers)]#tf.nn.rnn_cell.MultiRNNCell([lstm_cell_bw] * self.num_layers)

        # get the length of each sample
        self.length = tf.reduce_sum(tf.sign(self.inputs), reduction_indices=1)
        self.length = tf.cast(self.length, tf.int32)
        # forward and backward
        self.outputs, _, _ = rnn.static_bidirectional_rnn(#rnn.bidirectional_dynamic_rnn(
            lstm_cells_fw,
            lstm_cells_bw,
            self.inputs_emb,
            dtype=tf.float32,
        ...
        '''
        log_likelihood,self.transitions= tf.contrib.crf.crf_log_likelihood(self.tags_scores, self.targets,self.length)
    ...
    def forward(self, observations, transitions, length, is_viterbi=True, return_best_seq=True):
        tags=self.num_classes+1
        length = tf.reshape(length, [self.batch_size])
        transitions = tf.reshape(tf.concat([transitions] * self.batch_size,0), [self.batch_size, tags,tags])
        observations = tf.reshape(observations, [self.batch_size, self.num_steps + 2, tags, 1])
        ...
        for t in range(1, self.num_steps + 2):
            previous = tf.reshape(previous, [self.batch_size,tags, 1])
            current = tf.reshape(observations[t, :, :, :], [self.batch_size, 1, tags])
            alpha_t = previous + current + transitions
            if is_viterbi:
                max_scores.append(tf.reduce_max(alpha_t, reduction_indices=1))
                max_scores_pre.append(tf.argmax(alpha_t, dimension=1))
            alpha_t = tf.reshape(self.logsumexp(alpha_t, axis=1), [self.batch_size, tags, 1])
            alphas.append(alpha_t)
            previous = alpha_t           
            
        alphas = tf.reshape(tf.concat(alphas,0), [self.num_steps + 2, self.batch_size, tags, 1])
        alphas = tf.transpose(alphas, [1, 0, 2, 3])
        alphas = tf.reshape(alphas, [self.batch_size * (self.num_steps + 2), tags, 1])

        last_alphas = tf.gather(alphas, tf.range(0, self.batch_size) * (self.num_steps + 2) + length)
        last_alphas = tf.reshape(last_alphas, [self.batch_size,tags, 1])

        max_scores = tf.reshape(tf.concat(max_scores,0), (self.num_steps + 1, self.batch_size, tags))
        max_scores_pre = tf.reshape(tf.concat(max_scores_pre,0), (self.num_steps + 1, self.batch_size,tags))
    ...
    def train(self, sess, save_file, X_train, y_train, X_val, y_val):
        ...
        for epoch in range(self.num_epochs):
            ...
            for iteration in range(num_iterations):
                ...
                row = y_train_batch.shape[0]
                col = y_train_batch.shape[1]
                y_train_weight_batch = np.random.rand(row , col )
                #y_train_weight_batch = 1 + np.array((y_train_batch == label2id['ECT']) | (y_train_batch == label2id['ecn']), float)
                ...
                if iteration % 100 == 0:
                    ...
                    row=y_val_batch.shape[0]
                    col=y_val_batch.shape[1]
                    y_val_weight_batch = np.random.rand(row,col)
                    #y_val_weight_batch = 1 + np.array((y_val_batch == label2id['ECT']) | (y_val_batch == label2id['ECO']), float)
                    ...
                    if f1_val > self.max_f1:
                        self.max_f1 = f1_val
                        save_path = saver.save(sess,'checkpoints/'+ save_file)
    ...
    def predictBatch(self, sess, X, X_str, id2label):
        ...
        for i in range(len(predicts)):
            ...
            y_pred = ''.join([id2label[val] for val in predicts[i] if val != 100 and val != -1 and val!=0])
    ...
    def evaluate(self, X, y_true, y_pred, id2char, id2label):
        ...
        for i in range(len(y_true)):
            ...
            y = ''.join(str(id2label[val]) for val in y_true[i] if val!=0)
            y_hat = ''.join([id2label[val] for val in y_pred[i]  if val not in [-1,100,0]])
            #print('test sentence:' + x)
            #print('true tags:'+y)
            #print('predict tags:' + y_hat)
            true_labels = helper.extractLabel(x, y,'ECT')
            pred_labels = helper.extractLabel(x, y_hat,'ECT')
